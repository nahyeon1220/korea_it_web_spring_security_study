...
public class SecurityConfig {
    ...
    //CORS: 브라우저가 보안상 다른 도메인의 리소스 요청을 제한하는 정책
    //기본적으로 브라우저는 같은 출처(Same-Origin)만 허용
    @Bean
    public CorsConfigurationSource corsConfigurationSource() {
        CorsConfiguration corsConfiguration = new CorsConfiguration();
        //요청을 보내는 쪽의 도메인(사이트 주소)을 허용
        corsConfiguration.addAllowedOriginPattern(CorsConfiguration.ALL); //ALL이 붙었기 때문에 모든 도메인 허용
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        //JWT 방식 또는 무상태(Stateless) 인증이기 때문에 세션이 없고, 쿠키도 안 쓰고, 토큰 기반이어서
        //CSRF 공격 자체가 성립되지 않음 -> 꺼둠
